...
class RfbInput {
     ...
     */
    sendKeyEventImmediate(keysym, down) {
        // 先刷新缓冲队列，保证事件顺序
     ...
     */
    enqueueModifierKey(keysym, down) {
        if (down) {
            if (this.remoteModifiers.has(keysym))
                return; // 已按下，跳过
     ...
     */
    buildKeyEventMessage(keysym, down) {
        const msg = Buffer.alloc(8);
        msg[0] = types_1.ClientMsgType.KeyEvent; // 4
        msg[1] = down ? 1 : 0;
        msg[2] = 0; // padding
        msg[3] = 0; // padding
    ...
    sendPointerEvent(buttonMask, x, y) {
        ...
        msg[0] = types_1.ClientMsgType.PointerEvent; // 5
        msg[1] = fullMask & 0xFF;
     ...
     */
    sendPointerEventRaw(buttonMask, x, y) {
        const msg = Buffer.alloc(6);
        msg[0] = types_1.ClientMsgType.PointerEvent; // 5
        msg[1] = buttonMask;
        msg.writeUInt16BE(Math.max(0, x), 2);
     ...
     * 更新鼠标按键掩码
     */
    updateButtonMask(button, pressed) {
        if (pressed) {
...
exports.RfbInput = RfbInput;
RfbInput.FLUSH_INTERVAL_MS = 8; // ~120Hz 的刷新率
...
/** 修饰键集合（用于快速判断） */
RfbInput.MODIFIER_KEYSYMS = new Set([
