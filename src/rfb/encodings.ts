...
export class EncodingDecoders {
      ...
      for (let tx = 0; tx < width; tx += 64) {
        ...
        if (subType >= 1 && subType <= 127) {
          ...
          if (isRLE) {
            ...
            const packedLen = this.blitPackedPalette(
              buffer, offset + consumed, palette, bits,
              tw, th, fb, tx, ty, width
            );
            if (packedLen === null) return null;
            consumed += packedLen;
          } else {
  ...
  /** Tight zlib 累积流 */
  private tightZlibAccumulated: Buffer = Buffer.alloc(0);
  ...
  private decodeTight(
    ...
    // 读取压缩数据
    if (buffer.length < offset + compressedLen) return null;
  ...
  /** 从帧缓冲中获取像素 */
  private getPixel(fb: Buffer, x: number, y: number, fbWidth: number): number[] {
