...
import { EncodingType, PixelFormat } from './types';
...
export class EncodingDecoders {
  private inflatePool: Map<number, zlib.Inflate> = new Map();
  ...
  private decodeZRLE(
    ...
    if (buffer.length < offset + 4 + compressedLen) return null;
      ...
      for (let tx = 0; tx < width; tx += 64) {
        ...
        if (subType >= 1 && subType <= 99) {
          ...
          for (let i = 0; i < paletteSize; i++) {
            if (buffer.length < offset + consumed + 3) return null;
   ...
   */
  private readCPixel(buffer: Buffer, offset: number, format: PixelFormat): number[] {
