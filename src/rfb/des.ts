...
function bytesToBits(buf: Buffer, bytes: number): number[] {
  const bits: number[] = new Array(bytes * 8);
...
/** 生成 16 个 48 位子密钥 */
function createSubKeys(key: Buffer): number[][] {
...
/** DES 单块加密（8 字节 -> 8 字节） */
function encryptBlock(block: Buffer, subKeys: number[][]): Buffer {
  ...
  for (let round = 0; round < 16; round++) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 3; j >= 0; j--) sOutput.push((value >> j) & 1);
    }
